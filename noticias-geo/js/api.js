...
/**
 * Trae UNA sola tanda de noticias (una única llamada a la API,
 * para no chocar con el límite de peticiones por segundo del plan
 * gratuito). El resultado se reparte luego entre las secciones del
 * sitio en main.js / article.js.
 * Si no hay key, o la API falla, devuelve datos de ejemplo.
 */
let _newsCache = null; // evita pedir dos veces en la misma carga de página

async function fetchAllNews() {
  if (_newsCache) return _newsCache;

  if (!CONFIG.GNEWS_API_KEY) {
    _newsCache = MOCK_ARTICLES;
    return _newsCache;
  }

  const url = `${CONFIG.GNEWS_BASE_URL}?q=${encodeURIComponent(CONFIG.QUERY)}` +
  ...
  try {
    ...
    _newsCache = data.articles.map((a, i) => ({
      id: `api-${i}`,
      title: a.title,
    ...
    console.warn(`[fetchAllNews] Falló la API (${err.message}); usando datos de ejemplo.`);
    _newsCache = MOCK_ARTICLES;
  }
